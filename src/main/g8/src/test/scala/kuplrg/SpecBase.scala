...
trait SpecBase extends AnyFlatSpec with BeforeAndAfterAll {
  ...
  private def newTest[T](
    ...
    exprCode should cond.getOrElse("be terminated normally") in {
      ...
      val msg = try
        Await.result(Future(Try(result)), timeLimit.second).get
      catch
...
private object SpecBase:
  def checkImpl[T](
    ...
    '{ $doit($exprCode, $expr, $weight, $timeLimit) }
  ...
  def testImpl[T](
    ...
    '{ $doit($exprCode, $answerCode, $expr, $answer, $weight, $timeLimit) }
  ...
  def testExcImpl[T](
    ...
    $doit(${ normCode(expr) }, $expr, $errMsg, $weight, $timeLimit)
  }
